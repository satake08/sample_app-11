...
#エラーメッセージを表示→app/views/new.html.erb内「<h1>新規投稿</h1>」の下に「<% if @list.errors.any? %>～<% end %>」追加
...
#rails cを使う
#rails cで画像を扱う場合は複雑になるため、Listモデルに設定してあるimageのバリデーションをコメントアウトしタイトルと本文のみで投稿できるように変更
#app/models/list.rb内「validates :image, presence: true」を削除or＃つけてコメントアウト(終わったら元に戻す)
#起動→ターミナルで「rails c」で実行→「Running～3.1.2 :001 >」と表示されれば「rails c」が起動されたということ
#「3.1.2 :001 >」この後に実行したいコマンド入力、終了コマンド→「exit」
#標準のconsoleのツールであるirbからpryに変更(pryはirbの「強化版」使い勝手が良くなる)
 #Gemfile内「group :development, :test do..end」内の「end」の上に「gem 'pry-rails'」記述後、ターミナルで「bundle install」実行
 #ターミナルで「rails c」実行→「Running～[1] pry(main)>」と表示されれば成功、ここまででirbからpryへの変更は完了
#実際にコマンドを入力
 #レコードの保存→ターミナル「pry(main)> 」の後に「List.create(title:"railsを学ぼう",body:"rails cについて")」実行
 #全レコードの取得→ターミナル「pry(main)> 」の後に「List.all」実行(「List.all」は今までにcreateされたレコードを全て取得する事ができる)
  #取得したレコードなどが多い場合は、表示しきれずに「：」という表記が出ることがある
  #Enterキーを押すことで続きのレコードが表示できるようになるが、コンソールのコマンドを受け付ける状態であるpry(main)がなくなり、
  #別のコマンドが実行できない状態になる。これを終了する際には「Q」キーを押す必要がある
 #指定したレコードを取得→ターミナル「pry(main)> 」の後に「List.find(1)」実行(idが1のレコードを取得したい場合)
 #データの削除→ターミナル「pry(main)> 」の後に「List.find(1).destroy」実行(idを指定してデータを削除したい時)
 #ターミナル「pry(main)> 」の後に「List.find(1)」実行(削除されたか確認)
 
#レイアウトを整える
#HTMLファイルを編集
 #index.html.erbの編集→app/views/lists/index.html.erb内「<div class="list-wrap">～</div>」に書き換え
 #application.html.erbの編集→app/views/layouts/application.html.erb内「<header>～<main>、</main>」追加
#CSSファイルを編集
 #application.cssの編集→app/assets/stylesheets/application.css内「* {～}」追加
#フォントをデザイン
 #app/views/layouts/application.html.erb内の「</head>」の上に「<link href="https://fonts.googleapis.com/css?family=Kaushan+Script" rel="stylesheet">」追加
 #app/assets/stylesheets/application.css内数か所追記
#CSSで文字を省略して表示
 #app/assets/stylesheets/application.css内数か所追記